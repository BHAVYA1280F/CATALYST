...
def _clean_line(line):
    ...
    line = line.strip()
    # Strip a single trailing sentence-ending period (but not "..." or a
    # line that's just punctuation). Without this, lines like
    # "Add 5 to score." fail to match patterns anchored with $, and
    # "Create x with 10." silently turns 10 into the float 10.0.
    if len(line) > 1 and line.endswith('.') and not line.endswith('..'):
        line = line[:-1].strip()
    return line
...
def _parse_block(items, index=0, base_indent=None, known=None, stop_keywords=()):
    ...
    while index<len(items):
        ...
        # FOR
        m=re.match(r'^for\s+([A-Za-z_]\w*)\s+from\s+(.+?)\s+to\s+(.+?)\s*(?::\s*(.*))?$',line,re.I)
        if m:
            var,a,b,inline=m.groups(); index+=1; body=[]
            known.add(var)
            if inline and inline.strip(): body=[_statement(inline,known)]
            elif index<len(items) and items[index][0]>base_indent: body,index=_parse_block(items,index,items[index][0],known)
            else: raise ParseError(f"FOR on line {ln} needs a body")
            steps.append({"operation":"FOR","variable":var,"start":_value(a,known),"end":_value(b,known),"body":body}); continue
        ...
        # WHILE
        m=re.match(r'^while\s+(.+?)\s*(?::\s*(.*))?$',line,re.I)
        if m:
            cond,inline=m.groups(); index+=1; body=[]
            if inline and inline.strip(): body=[_statement(inline,known)]
            elif index<len(items) and items[index][0]>base_indent: body,index=_parse_block(items,index,items[index][0],known)
...
for level,concepts in LEVELS.items():
    for concept in concepts:
        examples={
            'Variables':'Create age with 10.', 'Numbers':'Create a number score with 95.', 'Strings':'Create a string name with "Tanvi".', 'Booleans':'Create a boolean happy with true.', 'Typecasting':'Create age with 10.\nConvert age to string.',
            'Arithmetic operators':'Set total to 10 + 5 * 2.', 'Comparison operators':'Create age with 15.\nIf age is greater than 10: Print "Older".', 'Logical operators':'Create age with 15.\nCreate a boolean happy with true.\nIf age > 10 and happy == true: Print "Yes".', 'Assignment operators':'Create score with 0.\nAdd 5 to score.',
            'if / elif / else':'Create score with 85.\nIf score >= 90: Print "A"\nElif score >= 60: Print "B"\nElse: Print "C".', 'for loops':'For i from 1 to 5: Print i.', 'while loops':'Create x with 0.\nWhile x < 5: Add 1 to x.',
            'Classes & objects':'Create a class Animal:\n    Create a method speak:\n        Print "Hello".\nCreate an object dog of Animal.\nCall speak on dog.', 'Inheritance':'Create a class Animal:\n    Create a method speak:\n        Print "...".\nCreate a class Dog that inherits Animal:\nCreate an object rex of Dog.\nCall speak on rex.', 'Polymorphism':'Create a class Animal:\n    Create a method speak:\n        Print "...".\nCreate a class Dog that inherits Animal:\n    Create a method speak:\n        Print "Woof".\nCreate an object rex of Dog.\nCall speak on rex.',
            'try / except':'Try:\n    Raise exception with "Something went wrong".\nExcept:\n    Print "Handled".', 'Raising exceptions':'Try:\n    Raise exception with "Invalid value".\nExcept:\n    Print "Caught it".'}
